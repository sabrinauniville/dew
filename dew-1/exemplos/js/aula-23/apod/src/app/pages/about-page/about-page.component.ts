...
@Component({
  ...
  template: `
    <section class="contact-page">
      <h2>Fale conosco</h2>
      <p>Preencha o formulário abaixo. Esta versão é somente front-end, então a mensagem não será enviada para nenhum servidor.</p>
      ...
      <form [formGroup]="contactForm" (ngSubmit)="submitForm()" novalidate>
        ...
        @if (contactForm.get('name')?.touched && contactForm.get('name')?.invalid) {
          <small class="error">Informe um nome com pelo menos 3 caracteres.</small>
        }
        ...
        <textarea id="message" rows="5" formControlName="message" placeholder="Escreva sua mensagem"></textarea>
        @if (contactForm.get('message')?.touched && contactForm.get('message')?.invalid) {
          <small class="error">A mensagem deve ter pelo menos 10 caracteres.</small>
        }
      ...
      @if (submitted) {
        <p class="success">Mensagem enviada com sucesso! (simulação front-end)</p>
      }
    </section>
  `,
  styles: [
    `
      .contact-page { max-width: 480px; margin: 0 auto; }
      ...
      button { width: fit-content; padding: 0.7rem 1rem; border: 0; border-radius: 8px; background: #0f172a; color: white; cursor: pointer; }
      button:disabled { opacity: 0.6; cursor: not-allowed; }
      .error { color: #b91c1c; display: block; }
      .success { margin-top: 1rem; color: #166534; font-weight: 600; }
    `,
  ],
})
